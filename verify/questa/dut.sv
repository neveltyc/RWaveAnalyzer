`timescale 1ns / 1ps
// Differential fixture: the same driver/load shapes as verify/npi/dut.sv, so the
// two backends can be asked the same questions about the same RTL.
//
// The FSDB dump is guarded rather than removed: with +define+FSDB this compiles
// under VCS and produces tb.fsdb for the NPI side, and without it under Questa,
// which logs through `add log -r /*` instead.
...
module core (
  ...
  // Two drivers on one net, so a multi-driver answer has to survive.
  logic       oe_a, oe_b;
  wire        shared;
  assign shared = oe_a ? 1'b1 : 1'bz;
  assign shared = oe_b ? 1'b0 : 1'bz;
  assign oe_a = (state == RUN);
  assign oe_b = (state == DONE);
endmodule
...
module tb;
  ...
  // testbench-procedural drive: an RTL fan-in cannot see this
  initial begin
    b.vld  = 0;
    b.data = 8'h00;
    #12 rst_n = 1;
