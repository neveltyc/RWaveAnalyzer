...
impl<R: BufRead + Seek> FstReader<R> {
    ...
    pub fn read_signals(
        ...
        // The include-mask is indexed by `FstSignalHandle::get_index()`. Its
        // upper bound is normally `max_var_id_code`, but some writers (observed
        // with VCS-generated FSTs) emit value-change geometry whose handle
        // indices exceed the declared `max_var_id_code`. Sizing the mask by
        // either `signals.len()` or `max_var_id_code` alone then overflows on
        ...
        let signal_mask = if let Some(signals) = &filter.include {
            let mut signal_mask = BitMask::repeat(false, mask_size);
            for sig in signals {
